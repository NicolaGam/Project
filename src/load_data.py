...
def load_data():
    ...
    e = pd.read_excel(os.path.join(config.RAW_DATA_PATH,
                    'Real estate valuation data set.xlsx'))
    e.drop_duplicates(inplace=True)
    
    e.columns = ['No', 'date','HouseAge','DistancetoMRT','NumOfConvStore','Latitude','Longitude','HousePrice']
    e = e[['date', 'HouseAge','DistancetoMRT','NumOfConvStore','Latitude','Longitude','HousePrice']]
    ...
    e.to_sql(config.RAW_TABLE, conn, if_exists='replace', index=False)
